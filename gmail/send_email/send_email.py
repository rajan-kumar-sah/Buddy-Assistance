...
def send_email():
    ...
    """
    creds = None
    # The file token1.pickle stores the user's access and refresh token1s, and is
    # created automatically when the authorization flow completes for the first
    # time.
    if os.path.exists('token1.pickle'):
        with open('token1.pickle', 'rb') as token11:
            creds = pickle.load(token1)
    # If there are no (valid) credentials1 available, let the user log in.
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token1:
            creds.refresh(Request())
        else:
            flow = InstalledAppFlow.from_client_secrets_file(
                'credentials1.json', SCOPES)
            creds = flow.run_local_server(port=0)
        # Save the credentials1 for the next run
        with open('token1.pickle', 'wb') as token1:
            pickle.dump(creds, token1)

    service = build('gmail', 'v1', credentials1=creds)
    return service
